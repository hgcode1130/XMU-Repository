    ...
    , ui(new Ui::MainWindow)
{
    ...
    view = new QGraphicsView(scene, this);		//创建视图对象
    customWidget = new CustomWidget();		// 创建自定义Widget对象
    ...
    //连接按钮的信号和槽函数
    connect(thread,&QThread::started,customWidget->key,&KeyDevice::getKeys);			//获取按键的线程
    connect(customWidget->key,&KeyDevice::KeysState,this,&MainWindow::changeKey);		//改变按键状态
    connect(customWidget->m_exitButton1,&QPushButton::clicked, this, &MainWindow::on_exit_clicked);	//点击退出图标
    thread->start();
}
