#单周期MIPS处理器（多重中断）测试程序        test7-1.asm                         冒泡法排序（降序排列，从大到小）                       
...
#中断堆栈的起始地址：  					1024 = 400H                         		RAM对应100H   （400H/4=100H）


main:
	addi $sp,$zero,1024  	  #设置中断服务程序堆栈的地址
...
IRA:                                                         #IRA中断服务程序的入口地址：3400H       ROM对应100H         

	sw $a0, 0($sp)		#push registers  需要保留中断程序用到的寄存器                 $a0    $v0 
	addi $sp, $sp, 4
	sw $v0, 0($sp)
	addi $sp, $sp, 4
	...
	nop
 	...
 	addi   $a0,$0,0x6666         	#display  00006666
	addi   $v0,$0,34           	
	syscall      
