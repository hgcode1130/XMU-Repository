#MIPS程序    test10-2.asm    冒泡法排序（升序排列，从小到大）   +  求累加和                    
...
#5个数（2、5、3、1、4）存放的起始地址：3072          			RAM对应300
...
main:
	addi $sp,$zero,3840  	  #设置中断服务程序堆栈的地址
 	...
 	addi $s0,$zero,2                        #第1个数=2（可以修改）保存到(3072+0)
 	sw $s0,3072($zero)

 	addi $s0,$zero,5                        #第2个数=5（可以修改）保存到(3072+4)
 	sw $s0,3076($zero)

 	addi $s0,$zero,3                        #第3个数=3（可以修改）保存到(3072+8)
 	sw $s0,3080($zero)

 	addi $s0,$zero,1                        #第4个数=1（可以修改）保存到(3072+12)
	sw $s0,3084($zero)
