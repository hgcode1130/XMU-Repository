package ch15.SerializationApps1;
...
public class CreateSequentialFile {
   public static void main(String[] args) {
      ...
      try(BufferedWriter output = 
         Files.newBufferedWriter(Paths.get("src/ch15/SerializationApps1/clients.xml"))) {
         //CTRL+ENTER +d
       ...
       //  while (input.hasNext()) { // loop until end-of-file indicator
            try {
               // create new record
               account = new Account(input.nextInt(),
                  input.next(), input.next(), input.nextDouble());

               // add to AccountList
              // account.getAccounts().add(record);
            } 
